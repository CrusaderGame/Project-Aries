...
void UUMultiplayerGameInstance::OnFindSessionsComplete(bool bWasSuccessful)
{
	...
	for (auto Result : SessionSearch->SearchResults)
	{
		...
		if (MatchType == FString("FreeForAll"))
		{
			if (GEngine)
			{
				GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Blue, FString::Printf(TEXT("Joining Match Type: %s"), *MatchType));
			}
...
void UUMultiplayerGameInstance::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
	if (!OnlineSessionInterface.IsValid())
	{
		return;
